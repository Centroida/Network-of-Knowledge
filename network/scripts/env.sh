...
export FABRIC_CA_CLIENT_ID_AFFILIATION=''
...
function initOrgVars {
   ...
   ROOT_CA_HOST=root-ca-${ORG}
   ROOT_CA_NAME=root-ca-${ORG}
   ROOT_CA_LOGFILE=$LOGDIR/${ROOT_CA_NAME}.log
   INT_CA_HOST=intermediate-ca-${ORG}
   INT_CA_NAME=intermediate-ca-${ORG}
   INT_CA_LOGFILE=$LOGDIR/${INT_CA_NAME}.log

   # Root CA admin identity
   ROOT_CA_ADMIN_USER=root-ca-${ORG}-admin
   ROOT_CA_ADMIN_PASS=admin
   ...
   ROOT_CA_INT_USER=intermediate-ca-${ORG}
