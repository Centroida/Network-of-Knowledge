...
function writeIntermediateCA {
   echo "  $INT_CA_NAME:
    ...
    environment:
      ...
      - PARENT_URL=http://$ROOT_CA_ADMIN_USER_PASS@$ROOT_CA_HOST:7054
...
function writeOrderer {
   ...
   echo "  $ORDERER_NAME:
    ...
    environment:
      ...
      - ENROLLMENT_URL=http://$ORDERER_NAME_PASS@$CA_HOST:7054
      - ORDERER_HOME=$MYHOME
    ...
    command: /bin/bash -c 'sleep 5s; /scripts/start-orderer.sh 2>&1 | tee /$ORDERER_LOGFILE'
    volumes:
      - ./scripts:/scripts
      - ./$DATA:/$DATA
    networks:
...
function writePeer {
   ...
   echo "  $PEER_NAME:
    ...
    environment:
      - FABRIC_CA_CLIENT_HOME=$MYHOME
      # - FABRIC_CA_CLIENT_TLS_CERTFILES=$CA_CHAINFILE
      - ENROLLMENT_URL=http://$PEER_NAME_PASS@$CA_HOST:7054
      - PEER_NAME=$PEER_NAME
      - PEER_HOME=$MYHOME
      - PEER_HOST=$PEER_HOST
